...
//-------- constants --------
    ...
    protected static void processVision(IEnvironmentSpace space, ISpaceObject obj, IExternalAccess agent) {
        ...
        final Set objects = ((Space2D) space).getNearObjects((IVector2) obj.getProperty(Space2D.PROPERTY_POSITION),
                new Vector1Double(vision));
        if (objects == null) {
            return;
        }
        agent.scheduleStep(new IComponentStep<Void>() {
            ...
            public IFuture<Void> execute(IInternalAccess ia) {
                BaseMobileAgent ba = (BaseMobileAgent) ia.getFeature(IPojoComponentFeature.class).getPojoAgent();
                for (Iterator<ISpaceObject> it = objects.iterator(); it.hasNext();) {
                    ...
                    if (so.getType().equals("target")) {
                        ...
                        System.out.println("New target seen: " + so.getId());
                    }
                }
                return IFuture.DONE;
            }
