 ...
 */
@Capability
@Plans({ @Plan(trigger = @Trigger(goals = { MovementCapability.Move.class,
        MovementCapability.Missionend.class }), body = @Body(MoveToLocationPlan.class)),
...
public class MovementCapability {
    ...
    /** The environment. */
    protected AbstractEnvironmentSpace env = (AbstractEnvironmentSpace) EnvironmentService
            .getSpace(capa.getAgent(), "myspace").get();

    /** The environment. */
    protected ISpaceObject myself = env.getAvatar(capa.getAgent().getDescription(),
            capa.getAgent().getModel().getFullName());

    /** The mission end. */
//  @Belief(dynamic=true, updaterate=1000) 
//    @Belief(updaterate = 1000)
//    protected boolean missionend = ((Long) env.getSpaceObjectsByType("homebase")[0].getProperty("missiontime"))
//            .longValue() <= getTime();
...
//        }
    ...
    protected long getTime() {
        IClockService cs = capa.getAgent().getFeature(IRequiredServicesFeature.class)
                .getLocalService(IClockService.class);
        // todo: capa.getAgent().getComponentFeature().getService() does not work in
        // init expressions only from plans :-(
